...
<page:param name="instructions">
    <p>
        <webwork:if test="/selectedParentOption">
            <webwork:text name="'admin.issuefields.customfields.reorder.parent'">
                <webwork:param name="'value0'"><strong><webwork:property
                        value="/fieldConfig/name"/></strong></webwork:param>
                <webwork:param name="'value1'"><strong><webwork:property
                        value="/customField/name"/></strong></webwork:param>
                <webwork:param name="'value2'"><strong><webwork:property
                        value="/selectedParentOption/value"/></strong></webwork:param>
            </webwork:text>
        </webwork:if>
        <webwork:else>
    ...
    <ul class="square">
        <li><a title="<webwork:text name="'admin.issuefields.customfields.sort.alphabetically'"/>"
               href="<webwork:property value="/selectedParentOptionUrlPrefix('sort')" /><webwork:property value="/selectedParentOptionId" />"><webwork:text
                name="'admin.issuefields.customfields.sort.alphabetically'"/></a></li>
        <li><a title="<webwork:text name="'admin.issuefields.customfields.view.custom.field.configuration'"/>"
               href="ConfigureCustomField!default.jspa?customFieldId=<webwork:property value="/customField/idAsLong"/>"><webwork:text
                name="'admin.issuefields.customfields.view.custom.field.configuration'"/></a></li>
    </ul>
    <webwork:if test="/cascadingSelect == true">
        <p>
            <webwork:text name="'admin.issuefields.customfields.choose.parent'"/>:
            <select name="<webwork:property value="./customFieldHelper/id" />"
                    onchange="return loadUri(this.value);">
                ...
                <webwork:if test="/selectedParentOptionId">
                    <webwork:if test="/selectedParentOption/parentOption">
                        <webwork:iterator value="/selectedParentOption/parentOption/childOptions"
                                          status="'rowStatus'">
                            <option value="<webwork:property value="./optionId" />"
                                    <webwork:if test="./optionId == /selectedParentOptionId">selected</webwork:if>>
        ...
        <table class="grid maxWidth minColumns">
            ...
            <webwork:iterator value="/displayOptions" status="'status'">
                <tr class="<webwork:if test="/hlOptions/contains(./value) == true">rowHighlighted</webwork:if><webwork:elseIf test="@status/odd == true">rowNormal</webwork:elseIf><webwork:else>rowAlternate</webwork:else>">
                    ...
                    <webwork:if test="/displayOptions/size > 1">
                        <td valign=top align=center nowrap>
                            <webwork:if test="@status/first != true">
                                <a id="moveToFirst_<webwork:property value="./optionId" />"
                                   href="<webwork:property value="/doActionUrl(.,'moveToFirst')" escape="false" />"><img
                                        src="<%= request.getContextPath() %>/images/icons/arrow_first.gif" border=0
                                        width=16 height=16
                                        title="<webwork:text name="'admin.issuefields.customfields.move.to.first.position'"/>"></a>
                                <a id="moveUp_<webwork:property value="./optionId" />"
                                   href="<webwork:property value="/doActionUrl(.,'moveUp')" escape="false" />"><img
                                        src="<%= request.getContextPath() %>/images/icons/arrow_up_blue.gif"
                                        border=0 width=16 height=16
                                        title="<webwork:text name="'admin.issuefields.customfields.move.this.option.up'"/>"></a></webwork:if>
                            <webwork:else>
                            <image src="<%= request.getContextPath() %>/images/border/spacer.gif" border=0 width=13
                                   height=14>
                                <image src="<%= request.getContextPath() %>/images/border/spacer.gif" border=0
                                       width=20 height=16>
                                    </webwork:else>
                                    <webwork:if test="@status/last != true">
                                    <a id="moveDown_<webwork:property value="./optionId" />"
                                       href="<webwork:property value="/doActionUrl(.,'moveDown')" escape="false" />"><img
                                            src="<%= request.getContextPath() %>/images/icons/arrow_down_blue.gif"
                                            border=0 width=16 height=16
                                            title="<webwork:text name="'admin.issuefields.customfields.move.this.option.down'"/>"></a>
                                    <a id="moveToLast_<webwork:property value="./optionId" />"
                                       href="<webwork:property value="/doActionUrl(.,'moveToLast')" escape="false" />"><img
                                            src="<%= request.getContextPath() %>/images/icons/arrow_last.gif"
                                            border=0 width=16 height=16
                                            title="<webwork:text name="'admin.issuefields.customfields.move.this.option.to.last'"/>"></a></webwork:if>
                                    <webwork:else>
                                    <image src="<%= request.getContextPath() %>/images/border/spacer.gif" border=0
                                           width=13 height=14>
                                        <image src="<%= request.getContextPath() %>/images/border/spacer.gif"
                                               border=0 width=20 height=16></webwork:else>
                        </td>

                        <ui:textfield name="/newPositionTextBoxName(./optionId)"
                                      label="text('admin.issuefields.customfields.new.option.position')"
                                      theme="'single'" value="/newPositionValue(./optionId)" size="'2'">
                    ...
                    <webwork:param name="'value0'"><webwork:property value="./value" /></webwork:param>
                               ...
                               href="<webwork:property value="/selectedParentOptionUrlPreifx" escape="false" /><webwork:property value="./optionId" />"><webwork:text
                                    name="'common.words.edit'"/></a>&nbsp;|
                        </webwork:if>
                        <webwork:if test="/defaultValue(./optionId/toString()) != true">
                            <a id="del_<webwork:property value="./value"/>"
                               href="<webwork:property value="/doActionUrl(.,'remove')" escape="false" />"><webwork:text
            ...
            <tr class="rowHeader" align="center">
                <td>&nbsp;
                    ...
                    <input type="hidden" name="selectedParentOptionId"
                           value="<webwork:property value="/selectedParentOptionId" />">
                </td>
                <td>
                ...
                <webwork:if test="./displayOptions/size > 1">
                    <td>
                        <input type="submit" name="moveOptionsToPosition"
                               value="<webwork:text name="'common.forms.move'"/>">
                    </td>
                </webwork:if>
                <td></td>
            </tr>
...
<p>
    <page:applyDecorator name="jiraform">
        ...
        <page:param
                name="buttons">&nbsp;<input type="button" value="Done" onclick="location.href='ConfigureCustomField!default.jspa?customFieldId=<webwork:property
                value="/customField/idAsLong"/>'">&nbsp;</page:param>
